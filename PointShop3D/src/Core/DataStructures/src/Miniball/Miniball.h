#ifndef __MINIBALL_H_
#define __MINIBALL_H_

//    Copright (C) 1999
//    $Revision: 1.1 $
//    $Date: 2004/02/04 16:49:59 $
//
...
//
    ...
    class Miniball {
        ...
        public:
            ...
            // checking
            float      accuracy (float& slack) const;
            bool        is_valid (float tolerance = 1e-15) const;
     };
    ...
    class Basis {
        private:
            ...
            float*             current_c;      // points to some c[j]
            float              current_sqr_r;
        ...
        public:
            ...
            float              excess (const Point& p) const;
   ...
   template <int d>
   float Miniball<d>::max_excess (It t, It i, It& pivot) const
   {
       const float *c = B.center(), sqr_r = B.squared_radius();
       float e, max_e = 0;
       for (It k=t; k!=i; ++k) {
           const float *p = (*k).begin();
           e = -sqr_r;
           for (int j=0; j<d; ++j)
               e += sqr(p[j]-c[j]);
           if (e > max_e) {
   ...
   template <int d>
   float Miniball<d>::squared_radius () const
   {
   ...
   template <int d>
   float Miniball<d>::accuracy (float& slack) const
   {
       float e, max_e = 0;
       int n_supp=0;
       Cit i;
   ...
   template <int d>
   bool Miniball<d>::is_valid (float tolerance) const
   {
       float slack;
   ...
   template <int d>
   const float* Basis<d>::center () const
   {
       return current_c;
   }
   
   template <int d>
   float Basis<d>::squared_radius() const
   {
   ...
   template <int d>
   float Basis<d>::excess (const Point& p) const
   {
